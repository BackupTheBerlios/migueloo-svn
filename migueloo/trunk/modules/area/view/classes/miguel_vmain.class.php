    ...
    function _categoryTable()
    {
    	...
    	if (is_array($category) && $category[0]['name']) {
        	...
        	for($i=0; $i < $countCategory; $i++) {
        		$elem = container();
        		$elem->add(html_a(Util::format_URLPath('listCourses/index.php', 'institution_id=' . $this->getViewVariable('institution_id') . '&faculty_id=' . $this->getViewVariable('faculty_id') . '&department_id=' . $this->getViewVariable('department_id') . '&area_id=' . $category[$i]['id']), $category[$i]['name'], null, '_top'));
