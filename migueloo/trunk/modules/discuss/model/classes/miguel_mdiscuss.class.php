...
class miguel_MDiscuss extends base_Model
{
	/**
	 * This is the constructor.
	 *
	 */
    function miguel_MDiscuss() 
    { 
	...
	function insertTopic($forum_id, $title, $norm, $desc)
	{
		$now = date('Y-m-d H:i:s');
		$iMyId = Session::getValue('USERINFO_USER_ID');

		$ret_val = $this->Insert('forum_topic', 
								'forum_id, forum_topic_title, forum_topic_poster, forum_topic_date, forum_topic_normative, forum_topic_description',
								array($forum_id, $title, $iMyId, $now, $norm, $desc)); 	
	...
	function getTopicDescription($forum_id, $topic_id)
    {
		$ret_sql = $this->Select('forum_topic',
                                 'forum_topic_description',
                                 "forum_topic_id = $topic_id AND forum_id = $forum_id"); 	

	    if ($this->hasError()) {
        	$ret_val = null;
        } else {
			$ret_val = $ret_sql[0]['forum_topic.forum_topic_description'];
		}
		return $ret_val;			 			   		
	}

	function getTopicNormative($forum_id, $topic_id)
    {
		$ret_sql = $this->Select('forum_topic',
                                 'forum_topic_normative',
                                 "forum_topic_id = $topic_id AND forum_id = $forum_id"); 	

	    if ($this->hasError()) {
        	$ret_val = null;
        } else {
			$ret_val = $ret_sql[0]['forum_topic.forum_topic_normative'];
		}
		return $ret_val;			 			   		
	}
	...
	{
		$arrUsers = $this->SelectMultiTable('user, person', 'user.user_id, user.user_alias',
											"user.user_id = person.person_id");
		...
		if ($this->hasError() || count($arrUsers) == 0) {
			...
			for ($i=0; $i<count($arrUsers);$i++) {
				$ret_val[$arrUsers[$i]['user.user_id']] = $arrUsers[$i]['user.user_alias'];
			}
		}
