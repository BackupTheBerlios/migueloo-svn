...
class miguel_VForum extends miguel_VMenu
{
        ...
        function addForumItemData($_name, $_desc, $_date, $_num, $_link)
        {
                ...
                $link = html_a(Util::format_URLPath('discuss/index.php',"status=list_topic&id_forum=$_link"),
                                                agt('Entrar'), 'p');
        ...
        function addHeadForum(&$div)
        {
                ...
                $titulo = html_p(agt('Foroxxxxxx').': '.$strForumName);
        ...
        function addTopic($title, $poster, $date, $post_poster, $post_time, $id_topic)
        {
                ...
                $nom = html_a(Util::format_URLPath('discuss/index.php',"status=list_post&id_forum=$id_forum&id_topic=$id_topic"),$title,'titulo03a');
        ...
        function addPost($poster, $time, $text, $ip, $id_post, $parent, $title, $_log)
        {
                ...
                $rowMail = html_td('ptabla01','',_HTML_SPACE);
    ...
    function left_section()
        {
                ...
                //Opciones
                $table->add_row(html_td('ptabla02','',html_a(Util::format_URLPath('discuss/index.php','status=list_topic&id_forum=1'),'Temas', 'titulo02a')));
                $table->add_row(html_td('ptabla02','',html_a(Util::format_URLPath('discuss/index.php','status=list_topic&id_forum=1'),'Presentación', 'titulo02a')));
                $table->add_row(html_td('ptabla02','',html_a(Util::format_URLPath('discuss/index.php','status=list_topic&id_forum=1'),'Participantes', 'titulo02a')));
                $table->add_row(html_td('ptabla02','',html_a(Util::format_URLPath('discuss/index.php','status=list_topic&id_forum=1'),'Recursos', 'titulo02a')));
                $table->add_row(html_td('','',_HTML_SPACE));
                //$table->add_row(html_td('ptabla02','',$this->addForm('discuss', 'miguel_forumMenuForm')));
                //$table->add_row(html_td('','',_HTML_SPACE));
                $table->add_row(html_td('ptabla02','',html_a(Util::format_URLPath('pageViewer/index.php','url=normas_debate.htm'), agt('Normas para el debate'), 'titulo02a')));
                ...
                switch($status) {
                        ...
                        case 'new':
                                if ($this->issetViewVariable('submit')) {
                                        ...
                                        $div->add($this->addForm('discuss', 'miguel_forumform'));
                                }
                                break;
                        case 'list_forum':
                        default:
