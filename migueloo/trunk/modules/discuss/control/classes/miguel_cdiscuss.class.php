...
class miguel_CDiscuss extends miguel_Controller
{
        function miguel_CDiscuss()
        {
                ...
                $this->setModelClass('miguel_MDiscuss');

                $this->setViewClass('miguel_VDiscuss');
        ...
        function processPetition()
        {
            ...
            switch($status) {
                    case 'new_post':
                        ...
                        case 'list_post':
                                ...
                                if ($this->issetViewVariable('submit')) {
									...
									$this->setViewVariable('arrUsersAlias', $this->obj_data->getUsersAlias());
					...
					case 'list_topic':
							...
							$this->clearNavBarr();
							$this->addNavElement(Util::format_URLPath('discuss/index.php', "status=list_topic&id_forum=$iId_forum"), agt('Debate'));
							break;
					case 'new':
                                $iId_forum = $this->getViewVariable('id_forum');
                                if ($this->issetViewVariable('submit')) {
					...
					case 'norm':
                        ...
                        $this->setViewVariable('topicInfo', $this->obj_data->getTopic($iId_topic));

						$this->setViewVariable('normative', $this->obj_data->getTopicNormative( $iId_forum, $iId_topic));
                        
                        $this->addNavElement(Util::format_URLPath('discuss/index.php', "status=norm&id_forum=$iId_forum&id_topic=$iId_topic"), agt('Normativa'));
						break;
					case 'topic':
					default:
                        $iId_forum = $this->getViewVariable('id_forum');
						...
						$this->setViewVariable('description', $this->obj_data->getTopicDescription( $iId_forum, $iId_topic));
                        
                        $this->addNavElement(Util::format_URLPath('discuss/index.php', "status=topic&id_forum=$iId_forum&id_topic=$iId_topic"), agt('Descripción'));
                }
                $this->setPageTitle("miguel Discuss area ");
                $this->setMessage('');
        ...
        {
                $title = $this->getViewVariable('nombre');
				$norm = $this->getViewVariable('normativa');
				$desc = $this->getViewVariable('descripcion');

                $this->obj_data->insertTopic($forum_id, $title, $norm, $desc);
        }
